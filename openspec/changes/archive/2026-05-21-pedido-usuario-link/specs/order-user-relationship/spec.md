...
El sistema SHALL requerir `usuario_id`, `direccion_id` y `forma_pago_codigo` al crear un pedido mediante `POST /api/v1/pedidos`. El sistema SHALL derivar el snapshot `cliente_nombre` del `Usuario.nombre` y el snapshot `direccion_snapshot` de la `DireccionEntrega` correspondiente. El cliente no SHALL enviar texto libre de nombre o dirección.

#### Scenario: Creación exitosa con usuario y dirección válidos
...
- **THEN** el sistema responde `201 Created` con el pedido incluyendo `usuario_id`, `direccion_id`, `forma_pago_codigo`, `cliente_nombre` (snapshot del usuario) y `direccion_snapshot` (texto formateado de la dirección)
...
- **THEN** el sistema responde `200 OK` con todos los pedidos (comportamiento actual sin cambios)
